...
public abstract class AbstractDrakkarEntity extends AbstractBannerUser {
   ...
   public double icebreakcounter;

   public AbstractDrakkarEntity(EntityType<? extends AbstractDrakkarEntity> entityType, World worldIn) {
      super(entityType, worldIn);
      this.icebreakcounter = (Double)SmallShipsConfig.DrakkarIceBreakSpeed.get();
   }
   ...
   private void breakIce() {
      if (this.icebreakcounter == 0.0D) {
            ...
            for(int i = start.func_177958_n(); i <= end.func_177958_n(); ++i) {
               for(int j = start.func_177956_o(); j <= end.func_177956_o(); ++j) {
                  for(int k = start.func_177952_p(); k <= end.func_177952_p(); ++k) {
                     ...
                     if (blockstate.func_177230_c() instanceof IceBlock) {
         ...
         if (hasBroken) {
            this.field_70170_p.func_184148_a((PlayerEntity)null, this.func_226277_ct_(), this.func_226278_cu_(), this.func_226281_cx_(), SoundEvents.field_187561_bM, SoundCategory.BLOCKS, 1.0F, 0.9F + 0.2F * this.field_70146_Z.nextFloat());
         }
   ...
   public void updateMotion() {
      ...
      double d2 = 0.0D;
      double DrakkarTurnFactor = (Double)SmallShipsConfig.DrakkarTurnFactor.get();
      this.momentum = 1.0F;
      ...
      if (this.previousStatus == TNBoatEntity.Status.IN_AIR && this.status != TNBoatEntity.Status.IN_AIR && this.status != TNBoatEntity.Status.ON_LAND) {
         ...
         this.deltaRotation = (float)((double)this.deltaRotation * (double)(this.momentum - this.passengerfaktor) * DrakkarTurnFactor);
         if (d2 > 0.0D) {
   ...
   protected void controlBoat() {
      double DrakkarSpeedFactor = (Double)SmallShipsConfig.DrakkarSpeedFactor.get();
      ...
      if (this.func_184207_aI()) {
         ...
         if (sailstate != 0) {
            switch(sailstate) {
            case 1:
               f = (float)((double)f + 0.03999999910593033D * DrakkarSpeedFactor * 1.0D / 4.0D);
               if (this.forwardInputDown) {
                  f = (float)((double)f + 0.019999999552965164D * DrakkarSpeedFactor);
               }
               break;
            case 2:
               f = (float)((double)f + 0.03999999910593033D * DrakkarSpeedFactor * 2.0D / 4.0D);
               if (this.forwardInputDown) {
                  f = (float)((double)f + 0.019999999552965164D * DrakkarSpeedFactor);
               }
               break;
            case 3:
               f = (float)((double)f + 0.03999999910593033D * DrakkarSpeedFactor * 3.0D / 4.0D);
               if (this.forwardInputDown) {
                  f = (float)((double)f + 0.019999999552965164D * DrakkarSpeedFactor);
               }
               break;
            case 4:
               f = (float)((double)f + 0.03999999910593033D * DrakkarSpeedFactor * 1.0D);
               if (this.forwardInputDown) {
                  f = (float)((double)f + 0.019999999552965164D * DrakkarSpeedFactor);
               }
            }
         ...
         if (this.backInputDown) {
            f = (float)((double)f - 0.004999999888241291D * DrakkarSpeedFactor);
         }

         if (this.forwardInputDown && sailstate == 0) {
            f = (float)((double)f + 0.03999999910593033D * DrakkarSpeedFactor);
         }
   ...
   public boolean func_70097_a(DamageSource source, float amount) {
      double DrakkarHealth = (Double)SmallShipsConfig.DrakkarHealth.get();
      if (this.func_180431_b(source)) {
         ...
         if (source == DamageSource.field_76367_g) {
            ...
            if (flag || (double)this.getDamageTaken() > DrakkarHealth) {
