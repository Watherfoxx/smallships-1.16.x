...
public abstract class AbstractGalleyEntity extends AbstractBannerUser {
   ...
   public void updateMotion() {
      ...
      double d2 = 0.0D;
      double GalleyTurnFactor = (Double)SmallShipsConfig.GalleyTurnFactor.get();
      this.momentum = 1.0F;
      ...
      if (this.previousStatus == TNBoatEntity.Status.IN_AIR && this.status != TNBoatEntity.Status.IN_AIR && this.status != TNBoatEntity.Status.ON_LAND) {
         ...
         this.deltaRotation = (float)((double)this.deltaRotation * (double)(this.momentum - this.passengerfaktor) * GalleyTurnFactor);
         if (d2 > 0.0D) {
   ...
   protected void controlBoat() {
      double GalleySpeedFactor = (Double)SmallShipsConfig.GalleySpeedFactor.get();
      ...
      if (this.func_184207_aI()) {
         ...
         if (sailstate != 0) {
            switch(sailstate) {
            case 1:
               f = (float)((double)f + 0.03999999910593033D * GalleySpeedFactor * 1.0D / 4.0D);
               if (this.forwardInputDown) {
                  f = (float)((double)f + 0.019999999552965164D * GalleySpeedFactor);
               }
               break;
            case 2:
               f = (float)((double)f + 0.03999999910593033D * GalleySpeedFactor * 2.0D / 4.0D);
               if (this.forwardInputDown) {
                  f = (float)((double)f + 0.019999999552965164D * GalleySpeedFactor);
               }
               break;
            case 3:
               f = (float)((double)f + 0.03999999910593033D * GalleySpeedFactor * 3.0D / 4.0D);
               if (this.forwardInputDown) {
                  f = (float)((double)f + 0.019999999552965164D * GalleySpeedFactor);
               }
               break;
            case 4:
               f = (float)((double)f + 0.03999999910593033D * GalleySpeedFactor * 1.0D);
               if (this.forwardInputDown) {
                  f = (float)((double)f + 0.019999999552965164D * GalleySpeedFactor);
               }
            }
         ...
         if (this.backInputDown) {
            f = (float)((double)f - 0.004999999888241291D * GalleySpeedFactor);
         }

         if (this.forwardInputDown && sailstate == 0) {
            f = (float)((double)f + 0.03999999910593033D * GalleySpeedFactor);
         }
   ...
   public boolean func_70097_a(DamageSource source, float amount) {
      double GalleyHealth = (Double)SmallShipsConfig.GalleyHealth.get();
      if (this.func_180431_b(source)) {
         ...
         if (source == DamageSource.field_76367_g) {
            ...
            if (flag || (double)this.getDamageTaken() > GalleyHealth) {
